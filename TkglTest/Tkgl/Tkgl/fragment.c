...
uniform vec3 iResolution;   // this encodes screen space XY data in absolute pixels
uniform vec4 iMouse;        // mouse control input
...
void main()
{
    vec2 mo = vec2(1.0,0.5);//iMouse.xy/iResolution.xy;
	float time = 0.0;// + iTime;
    
    vec3 tot = vec3(0.0);
    vec2 p = frag_color.xy;// (-iResolution.xy + 2.0*gl_FragCoord.xy) / iResolution.y;
